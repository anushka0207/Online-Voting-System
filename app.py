...
def view_results():
    ...
    # Organizing results by election
    election_winners = {}
    for election, candidate, vote_count in results:
        if election not in election_winners:
            election_winners[election] = (candidate, vote_count)  # Store the top candidate for each election

    return render_template("view_results.html", results=results, election_winners=election_winners)
...
def create_election():
    ...
    if request.method == 'POST':
        ...
        with sqlite3.connect("database.db") as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO elections (name) VALUES (?)", (election_name,))
            conn.commit()
